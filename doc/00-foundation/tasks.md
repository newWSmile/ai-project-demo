...
- [x] 选择并掌握一种 Python 环境/依赖工具：当前使用 `venv + pip`。
...
- [x] 用 Python HTTP Client（httpx）完成相同调用。
- [x] 分别实现同步和流式输出。
...
- [x] 环境安装和启动说明。
- [x] 一份 Java/Python 调用方式对比记录。

## 第 2 周：Spring AI 与 Python 辅助服务

- [x] 使用 Spring Boot 4.1.x 创建项目。
- [x] 使用 Spring AI 2.0.x BOM 管理依赖。
- [x] 接入第一个 ChatModel。
...
- [x] 编写 Java 服务调用 FastAPI 的示例（已验证：Java `/api/chat/python` 调用 FastAPI 并返回模型结果、Usage 和耗时）。
- [ ] 使用 Docker Compose 启动 Java 服务和 Python 服务。

## 完成标准

- [ ] 新环境按照 README 可在 30 分钟内完成启动。
- [x] Java 与 Python 均能完成模型调用。
